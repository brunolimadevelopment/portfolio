...
export const ProjectCard = ({ project }: ProjectCardProps) => {
    ...
    const tecnologies = project.tecnologies.map(x => x.name).join(', ') // junta todas as tecnologias em uma string só.
    ...
    return (
        <div className="group rounded-lg h-[436px] flex flex-col bg-gray-800 overflow-hidden border-2 border-gray-800 hover:border-emerald-500 opacity-70 hover:opacity-100 transition-all">
            ...
            <div className="flex-1 flex flex-col p-8">
                ...
                <span className="text-gray-300 text-sm font-medium block mt-auto truncate">{tecnologies}</span>
            </div>
        </div>
    )
}